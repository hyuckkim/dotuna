...
namespace DoTuna.Test
{
    public class ScribanRendererTest
    {
        ...
        [Fact]
        public async Task RenderThreadPageAsync_WithAttachment_SingleHTMLSetting_ImageTag()
        {
            // Arrange
            Setting.Instance.SingleHTML = true; // SingleHTML 설정 활성화
            ...
            Assert.Contains("test.png", html); // attachment 파일명 포함
            Assert.Contains("<img", html, StringComparison.OrdinalIgnoreCase); // 이미지 태그 포함
            Assert.DoesNotContain("data/", html, StringComparison.OrdinalIgnoreCase); // 이미지 경로를 포함하지 않음
        }
    }
}
