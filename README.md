...
## todo
파일 이름에 경로를 허용해서, 서브 폴더로 나눌 수 있게 할 생각.  
다음과 같은 것들을 고려해야함:  
- 상대 경로. 링크를 걸 때 html 파일과 링크 파일의 위치 차이를 계산해야 한다.
- 이미지. html 파일 경로에서 ./data를 생성해야 한다.
- css 파일. ../를 경로 길이만큼 만들어야 한다.
- 인덱스에서 html 파일은 크게 상관없을 것 같은데...? 그냥 경로에 / 넣을 수 있으니깐.